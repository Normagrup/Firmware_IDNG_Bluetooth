...
var vis = d3.select("#div1").append("svg:svg")
    .attr("width", width + margin[1] + margin[3])
    .attr("height", height + margin[0] + margin[2])
  .append("svg:g")
    .attr("transform", "translate(" + margin[3] + "," + margin[0] + ")");

d3.json("assets/data/arf.json", function(json) {
  root = json;
  root.x0 = height / 2;
  root.y0 = 0;
...
function update(source) {
  ...
  var newWidth = (maxDepth + 1) * 200; // 200px por nivel
  var maxNodesInLevel = Math.max(...Object.values(levelCounts));
  var newHeight = 300 + maxNodesInLevel * 70; // 30 px por nodo de separación

  tree.size([newHeight, newWidth]);
  
  d3.select("svg").attr("width", newWidth + margin[1] + margin[3]);
  d3.select("svg").attr("height", newHeight + margin[0] + margin[2]);
  ...
  var nodeEnter = node.enter().append("svg:g")
      ...
      .on("click", function(d) { if(d === root) { return; } else { toggle(d); update(d); }});
  ...
  nodeEnter.append('a')
      ...
      .attr("y", function(d) { return (d.children) ? -10 : 0; })
      .attr("dy", ".35em")
